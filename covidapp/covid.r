...
countrylist = c('Monaco',"Australia","UAE",'Ireland', 'US', 'South Korea', 'UK',"South Africa")
...
offical_pred_plot <- function(selection_var){
  ...
  p1 <- ggplot(metrics, aes(x = country, y = deaths_per_conf_perc, fill = deaths_per_conf_perc)) + 
  ...
  p1 <- p1 + geom_errorbar(aes(ymin = est_death_percent, ymax = est_death_percent),size = 1)
